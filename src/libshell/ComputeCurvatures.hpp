...
public:
     ...
     * @param [out] mean reference to a vector in which the mean curvature will be stored (resized if necessary)
     */
    void compute(const tMesh & mesh, Eigen::VectorXd & gauss, Eigen::VectorXd & mean,
                 Eigen::MatrixXd & pCurvature1, Eigen::MatrixXd & pCurvature2, const bool orderByAbs = true) const;


    void computePrincipalVectors(const tMesh & mesh, Eigen::MatrixXd & pCurvature1, Eigen::MatrixXd & pCurvature2, const bool orderByAbs = true) const;


    void computePrincipalCurvatures(const tMesh & mesh, Eigen::VectorXd & pCurvature1, Eigen::VectorXd & pCurvature2, const bool orderByAbs = true) const;
};
