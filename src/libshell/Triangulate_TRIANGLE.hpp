...
namespace TRIANGLE
{
    ...
    inline void remeshByArea(
        const Eigen::MatrixXd& V, // planar vertices -- only x/y cols considered
        const Eigen::MatrixXi& F,
        const Eigen::MatrixXd& A, // area constraint in the plane
        const std::string flags,
        Eigen::MatrixXd& V2,
        Eigen::MatrixXi& F2)
    {
        ...
        in.trianglelist = (int*)calloc(in.numberoftriangles*3,sizeof(int));
        for (unsigned i=0;i<F.rows();i++)
            for (unsigned j=0;j<3;j++)
                in.trianglelist[i*3+j] = F(i,j);

        in.trianglearealist = (double*)calloc(in.numberoftriangles,sizeof(double));
        for (unsigned i=0;i<F.rows();i++)
            in.trianglearealist[i] = A(i);
