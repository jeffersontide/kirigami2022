...
// addded GC
    MaterialProperties_Iso_Array(const Real E_in, const Real P_in, const Eigen::VectorXd & t_in, const Real rho_in)
    {
        const int nFaces = t_in.rows();

        materials.reserve(nFaces);
        for(int i=0;i<nFaces;++i)
            materials.emplace_back(E_in, P_in, t_in(i), rho_in);
    }
    //
